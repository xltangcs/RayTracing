...
glm::vec3 Renderer::ray_color(Ray& ray)
{
	...
	{
		bool isHitScene = m_ActiveScene->Hit(ray, 0.0001f, std::numeric_limits<float>::max(), payload);

		if (isHitScene) //hit obj
		{
			glm::vec3 color(0.0f);
			Ray outRay;
			glm::vec3 color_from_emission = GetMaterial(payload.MaterialIndex)->Emitted(payload.UV, payload.WorldPosition);
